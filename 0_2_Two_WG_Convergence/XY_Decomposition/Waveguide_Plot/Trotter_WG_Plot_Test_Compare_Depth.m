%Trotter_WG_Plot_Test_Compare_Depth.m
clc;
clear all;
close all;
...
for steps=1:max_steps
    ...
    f=WG_Plot2PColor_Compare( ket_string2,ket_string2,phi_i_red,phi_a_red,ts,ta,2,1,50);
    D=getframe(f);
    C{steps}=D.cdata;
end

v=VideoWriter(['trotter_mov_' num2str(n) '.avi'],'Uncompressed AVI');
v.FrameRate=2;
open(v)
for i=1:length(C)
    writeVideo(v,C{i});
