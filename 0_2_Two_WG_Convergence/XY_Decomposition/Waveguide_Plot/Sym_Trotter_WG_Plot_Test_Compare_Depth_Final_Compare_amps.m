%STrotter_WG_Plot_Test_Compare_Depth_Final_Compare_amps.m
...
for steps=1:max_steps
    ...
    [gate,step_gate trotter_name trotter_step_name]=Strang_Trotter(t,steps,name,circ_string,circ_string_step,n,0,gates_names,gates_indexes);
    ...
    [phi_a,ta]=DepthExpansion2Matrix_Return_Wave(phi0,other_gates(ind(2)),depth-1,s_e,d_e);
    ...
    ticker=[ta(1:2:end)+(ta(2)-ta(1))/2];
    ticker(end)=ta(end);
    ticker(1)=ta(1);
    ax2.XTick=ticker;
    ax2.XTickLabel=[0:length(ticker)];
    %xlabel('Steps')
    %cb=colorbar();
    %ylabel(cb,'p')
    ...
    if with_header==1
        matlab2tikz(['WG_Sym/sym_n_' num2str(n) '_s_' num2str(steps)  '_t_' num2str(t) '_comp.tex'],'standalone',true,'extraCode','\usepackage{braket}','parseStrings',false,'width','7cm','height','3cm')
    else
        matlab2tikz(['WG_Sym/sym_n_' num2str(n) '_s_' num2str(steps)  '_t_' num2str(t) '_comp.tex'],'parseStrings',false,'width','7cm','height','3cm')
    end
end