function h=Compare_Fidelity_Distribution_Circ_XY_All_Interaction()
    ...
    for i=1:length(n_m)
        ...
        for l=1:steps
            ...
            %if l==1
            %    Gates2Circ2Preview(step_gate,elem_gates,other_gates1,'xy_gate','gate=expansion',2,30);
            %end
            ...
            [U_approx sizes anc_sizes] = Expansion2Matrix(other_gates(ind(2)));
    ...
    save(['comp_fid_dist_N_' num2str(max(n_m)) '.mat'],'data');
    ...
    photon_slider=uicontrol('Parent',h,'Style','slider','Tag','ph_slide','Position',[1125 150 20 310],'Min',1,'Max',length(n_m),'Value',1,'SliderStep',[1/100 1/100]);
...
function Redraw(a,b,ax,h,data)
    ...
    %matlab2tikz(['Fidelity_Density/fidelity_density_N_' num2str(N) '.tex'],'standalone',true)
end