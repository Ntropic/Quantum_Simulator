...
name=['higher_compare_n_' num2str(min(n_m)) '_to_' num2str(max(n_m)) '_s_' num2str(steps) '_r_' num2str(rec_order) '_t_' num2str(t) '_pauli_gray'];
...
for i=1:length(n_m)
    ...
    timer=tic();
    %% Exact decomposition
    [Hij,s]=Gray_Exchange_Hamiltonian_Particles(n);
    %FockPrint(Hij)
    ...
    other_gates1=Comp_Gate_Merger(other_gates,init,main,connector,outit);
    other_gates1=Gates_Tables_Prepare(other_gates1,elem_gates);   
    ...
    for l=1:steps
        ...
        pattern=rec_order;
        gate_name=['H_{G,n=' num2str(n) '}^{(' num2str(rec_order+1) ')}'];
        circ_str=['H_{G,n=' num2str(n) '}^{(' num2str(rec_order+1) ')}'];
        ...
        other_gates2=Comp_Gate_Merger(other_gates1,trott_gate);
        ...
        gate_num(i,l)=length(other_gates2(ind(2)).exp_steps.index);
        s_q(i,l,1:2)=[0,0];
