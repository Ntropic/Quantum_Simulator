function [ U ] = U2MWG_Strang_Trotter( H,t,l,coupled_guides,coupling_strengths )
%U2MWG_Strang_Trotter Creates a multi Waveguide Unitarian from a single Waveguide
%Unitarian
...
if nargin==4
    ...
    if isa(H,'cell')
        ...
        Uij{2}=eye(size(H{1}));
        for i=1:length(H)
            Uij{1}=expm(-1i*H{i}*t/l/2)*Uij{1};
        end
        for i=length(H):-1:1
            Uij{2}=expm(-1i*H{i}*t/l/2)*Uij{2};
        end
        ...
        Uij={expm(-1i*H*t/l/2),expm(-1i*H*t/l)};
    ...
    if isa(H,'cell')
        for i=1:size(coupled_guides,1)-1
            ...
            for i=1:length(H)
                Uij{i,1}=expm(-1i*H{i}*t/l/2)*Uij{1};
            end
            for i=length(H):-1:1
                Uij{i,2}=expm(-1i*H{i}*t/l/2)*Uij{2};
            end
        end
        for i=1:size(coupled_guides,1)
            %Determine indexes
        ...
        for i=1:size(coupled_guides,1)-1
            Uij{i,1}=expm(-1i*coupling_strengths(i)*H*t/l/2);
            Uij{i,2}=expm(-1i*coupling_strengths(i)*H*t/l);
        end
        for i=1:size(coupled_guides,1)-1
            %Determine indexes
            indexes=[s/2*(coupled_guides(i,1)-1)+(1:s/2),s/2*(coupled_guides(i,2)-1)+(1:s/2)];
