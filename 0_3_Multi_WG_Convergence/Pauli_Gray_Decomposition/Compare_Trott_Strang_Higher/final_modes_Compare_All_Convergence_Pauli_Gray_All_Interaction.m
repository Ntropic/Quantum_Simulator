...
%matlab2tikz(['pauli_gray_convergence_N_' num2str(N) '_M_' num2str(M) '.tex'],'standalone',true,'parseStrings',false)
...
for i=1:len_sq
    ...
    if t(end)<x_lim(2)*10
        diff_t=diff(t);
        diff_t=diff_t(end);
        how_many=round((x_lim(2)*10-t(end))/diff_t);
        if how_many<1
            how_many=1;
        end
        n=length(t);
...
xlabel('\# Gates');
legend(names,'interpreter','latex')
%matlab2tikz(['pauli_gray_errors_N_' num2str(N) '_M_' num2str(M) '.tex'],'standalone',true,'parseStrings',false)
