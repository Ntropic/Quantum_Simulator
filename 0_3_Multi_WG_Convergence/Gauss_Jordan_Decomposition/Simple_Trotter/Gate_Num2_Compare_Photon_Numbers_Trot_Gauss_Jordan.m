%Gate_Num2_Compare_Photon_Numbers_Circ_Gauss_Jordan.m
%Try to reduce gate numbers by separating the gj decomposition into N parts
...
name=['mwg_trot_compare_n_' num2str(min(n_m)) '_to_' num2str(max(n_m)) '_s_' num2str(steps) '_t_' num2str(t) '_N_' num2str(max(N_m)) '_gj2'];
...
for h=1:length(N_m)
    ...
    for i=1:length(n_m)
        ...
        for m=1:n
            Hm{m}= Gray_Exchange_Hamiltonian_Particle_Numbers(n,m);
        end
        %% Exact decomposition
        %Hij=H2MWG(H,N);
        %FockPrint(Hij)
        ...
        for l=1:steps
            ...
            for m=1:n
                ...
                s_q(h,i,l,1)=s_q(h,i,l,1)+gate.single_gates;
                s_q(h,i,l,2)=s_q(h,i,l,2)+gate.double_gates;
            end
            drawnow;
        end 
    end
