...
for method=method2
    for time_step=time_step2
        for st=st2
            ...
            %% Plot
             h_c_wg=figure('Position',[100,100,1400,600],'name','Effort Surface');
             surf([1:7],[2:4],eff);
             hold on;
             surf([1:3],[4:6],eff2(3:5,:));
        end

        xlabel('$N_\text{max}$')
        zlabel({'Effort in steps'})
        ylabel('$M$')
        ...
        matlab2tikz(['effort_' exits{method} '_' num2str(time_step) '.tex'],'standalone',false,'parsestrings',false,'width','6cm','height','4cm')
        %matlab2tikz(['effort_' exits{method} '_' num2str(time_step) '_stand.tex'],'standalone',true,'parsestrings',false,'width','6cm','height','4cm')
    end
end