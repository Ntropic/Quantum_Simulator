...
c=[];
for l=1:steps
    for j=1:size(F_av_ind,1)
        c(j,:,l)=Order_Of_Convergence(1-F_av_ind(j,1:end,l),(1:7));
    end
end
c2=[];
for l=1:steps
    for j=1:size(F_av_ind2,1)
        c2(j,:,l)=Order_Of_Convergence(1-F_av_ind2(j,1:end,l),(1:3));
    end
end
...
for i=1:size(c,2)
    for j=size(c,3)-1:-1:1
        surf([j+1 j+2],i+(0:steps-1)*1/9,reshape(c(:,i,j:j+1),steps,2));
        hold on;
    end
end
for i=1:size(c2,2)
