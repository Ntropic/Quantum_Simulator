...
for method=method2
    for time_step=time_step2
        ...
        figure()
        %% Fit waveguide number increase in effort 
        for i=1:size(eff2,2)
            eff_now=eff2(:,i)';
            waveg_num=2:size(eff2,1)+1;           
            [xData, yData] = prepareCurveData(waveg_num,eff_now);
            ...
            mar={'b','r','g'};
            plot(wg_num{i}',eff_num{i}',mar{i})
            hold on;
            
            matlab2tikz(['wg_inc' exits{method} '_' num2str(time_step) '.tex'],'standalone',false,'parsestrings',false,'width','6cm','height','4cm')
        end
    end
end