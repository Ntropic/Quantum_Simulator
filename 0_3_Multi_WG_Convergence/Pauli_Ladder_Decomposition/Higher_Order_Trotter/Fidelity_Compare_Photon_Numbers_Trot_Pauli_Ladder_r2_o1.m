%Fidelity_Compare_Photon_Numbers_Trott_Gauss_Jordan.m
clear all;
close all;
clc;
...
rec_order=2;%:1;
        ...
        name=['mwg_higher_compare_fid_n_' num2str(min(n_m)) '_to_' num2str(max(n_m)) '_s_' num2str(steps) '_t_' num2str(t)  '_r_' num2str(rec_order)  '_N_' num2str(max(N_m)) '_pauli_ladder'];
        ...
        for h=1:length(N_m)
            ...
            for i=1:length(n_m)
                ...
                H_parts=Gray_Hamiltonian_Steps(n,'ladder'); %Gray coding with ladder ordering -> dense coding with ladder fidelity
                for l=1:steps
